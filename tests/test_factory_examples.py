...
from tests.dynamic_factories import (
    DynamicFactory,
    create_story_from_spec,
    create_member_from_spec,
)
...
def test_generate_minimal_story(factory):
    ...
    print(
        f"Minimal story has {len(minimal_story)} required fields: {list(minimal_story.keys())}"
    )
...
def test_generate_search_response(factory):
    ...
    search_response = factory.create_search_response(
        "StorySearchResult", total=50, page_size=20
    )
...
def test_custom_story_generation(factory):
    ...
    story = create_story_from_spec(
        factory, started=True, completed=False, story_type="bug"
    )
...
if __name__ == "__main__":
    ...
    search = factory.create_search_response(
        "StorySearchResult", total=100, page_size=10
    )
    print(
        f"   Total: {search['total']}, Items: {len(search['data'])}, Has next: {search['next'] is not None}"
    )

    print("\n4. Available definitions in spec:")
    definitions = list(factory.parser.definitions.keys())[:10]
    print(
        f"   {', '.join(definitions)}... (and {len(factory.parser.definitions) - 10} more)"
    )
