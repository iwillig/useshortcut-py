...
class DynamicDataGenerator:
    ...
    def generate_value(self, property_schema: Dict[str, Any], property_name: str = "") -> Any:
        ...
        if prop_type == 'string':
            return self._generate_string(property_name, prop_format, pattern, constraints)
    ...
    def _generate_string(self, name: str, format: Optional[str], pattern: Optional[str], 
        ...
        if format == 'uuid':
            ...
            return fake.date_time_between(start_date='-1y', end_date='now', tzinfo=timezone.utc).isoformat()
        elif format == 'date':
    ...
    def _generate_integer(self, name: str, format: Optional[str], constraints: Dict[str, Any]) -> int:
        ...
        elif 'time' in name.lower() and ('lead' in name.lower() or 'cycle' in name.lower()):
            return fake.random_int(3600, 604800)  # 1 hour to 1 week in seconds
        
        return fake.random_int(minimum, maximum)
    
    def _generate_number(self, name: str, format: Optional[str], constraints: Dict[str, Any]) -> float:
    ...
    def _generate_array(self, property_schema: Dict[str, Any], property_name: str) -> List[Any]:
    ...
    def generate_from_definition(self, definition_name: str, overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Generate data for a specific definition from the OpenAPI spec."""
        definition = self.parser.get_definition(definition_name)
        if not definition:
            raise ValueError(f"Definition '{definition_name}' not found in OpenAPI spec")
        ...
        for prop_name, prop_schema in properties.items():
            ...
            if prop_schema.get('x-nullable') and fake.boolean(chance_of_getting_true=30):
                continue
...
class DynamicFactory:
    ...
    def create_list(self, model_name: str, count: int = 5, **overrides) -> List[Dict[str, Any]]:
        """Create a list of mock instances."""
        return [self.create(model_name, **overrides) for _ in range(count)]
    
    def create_search_response(self, model_name: str, total: int = 10, page_size: int = 10) -> Dict[str, Any]:
        ...
        return {
            "total": total,
            "data": data,
            "next": f"/api/v3/search?next={fake.uuid4()}" if total > page_size else None
        }
    
    def create_response_from_path(self, path: str, method: str) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
